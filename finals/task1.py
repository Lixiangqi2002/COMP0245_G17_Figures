...
def model_setup(task, param_lst, deep_nn_2=False, deep_nn_3=False):
    ...
    # plt.figure(figsize=(12, 6))
    # for i in range(len(param_lst)):
    #     plt.plot(t, np.log(training_loss_runs[i]), linestyle='--', color=cmap(i),
    #              label=f'Training Loss {task}: {param_lst[i]}')
    #     plt.plot(t, np.log(testing_loss_runs[i]), linestyle=':', color=cmap(i + len(param_lst)),
    #              label=f'Testing Loss {task}: {param_lst[i]}')
    # plt.title('Log of Training and Testing Loss')
    # plt.xlabel('Time [s]')
    # plt.ylabel('MSE Loss')
    # plt.legend()
    # plt.savefig(file_path_new + '/dual_log_loss.png')
    # plt.show()
